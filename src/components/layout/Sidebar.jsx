    ...
    <aside className={`flex flex-col h-full bg-navy border-r border-white/10 ${mobile ? 'w-72' : 'w-64'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="text-sm font-semibold leading-tight" style={{ letterSpacing: '-0.02em' }}>
              <span className="text-white">Area</span><span style={{ color: '#10B981' }}>Mates</span>
            </div>
      ...
      <nav className="flex-1 p-3 space-y-4 overflow-y-auto">
        {NAV.map(({ section, links }) => (
          <div key={section}>
            <p className="text-xs font-medium text-white/20 uppercase tracking-widest px-3 pb-1.5">{section}</p>
